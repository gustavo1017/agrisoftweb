...
Public Class frmPlaneamiento_View
    Inherits BasePage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            ...
            If Session("frmPlaneamiento_Action_Action") = "NEW" Or Session("frmPlaneamiento_Action_Action") = "EDIT" Then
                Refresh()
            End If
        End If
    End Sub
    ...
    Protected Sub btnBorradoTotal_Click(sender As Object, e As EventArgs)
        ...
        If Session("frmPlaneamiento_View_NumeroParte") Is Nothing Then
            Exit Sub
        End If
        ...
        If Session("frmPlaneamiento_View_nRecords") IsNot Nothing Then
            nRecords = Convert.ToInt64(Session("frmPlaneamiento_View_nRecords"))
        End If
    ...
    Protected Sub btnEliminar_Click(sender As Object, e As EventArgs)
        If Session("frmPlaneamiento_View_IdCosto") Is Nothing Then
            Exit Sub
        End If
    ...
    Protected Sub btnExportarExcel_Click(sender As Object, e As EventArgs)
        ...
        Session.Add("frmExportParametros_ParamExport", "COSTOSWEB")
    ...
    Protected Sub grilla_RowDataBound(sender As Object, e As GridViewRowEventArgs)
        Select Case e.Row.RowType
            Case DataControlRowType.Header
                ...
                e.Row.Cells(5).Text = Resource1.str315
    ...
    Protected Sub grilla_SelectedIndexChanged(sender As Object, e As EventArgs)
        For Each row As GridViewRow In grilla.Rows
            If row.RowIndex = grilla.SelectedIndex Then
                ...
                Session.Add("frmPlaneamiento_View_IdCosto", row.Cells(0).Text)
                Session.Add("frmPlaneamiento_Codigo", row.Cells(0).Text)
                Session.Add("frmPlaneamiento_View_NumeroParte", row.Cells(2).Text)
                'Session.Add("frmCostos_StrIdenlace", row.Cells(0).Text)
                'Session.Add("frmPlaneamiento_View_ParametroSistema", row.Cells(4).Text)
            Else
    ...
    Public Function Delete() As Boolean
        Dim idCosto As String = Session("frmPlaneamiento_View_IdCosto").ToString()
    ...
    Private Function DeleteTotal() As Boolean
        ...
        Dim bolResult As Boolean = False
        Dim nroParte As String = Session("frmPlaneamiento_View_NumeroParte").ToString()
        Dim objBL As New GenericMethods("Fundo0")
        Dim DBconn As New ADODB.Connection
    ...
    Private Sub cargarDatosGrilla(ByVal sField As String, ByVal sText As String, Optional ByVal sText2 As String = "", Optional ByVal sOrder As String = "")
        ...
        Session.Add("frmPlaneamiento_View_nRecords", 0)
        ...
        Session.Add("frmPlaneamiento_View_IdCosto", Nothing)
        'End If

        Session.Add("frmPlaneamiento_Codigo", Nothing)
        Session.Add("frmPlaneamiento_StrIdenlace", Nothing)
        Session.Add("frmPlaneamiento_Action", "")
        Session.Add("frmPlaneamiento_View_nRecords", nRecords)
        Session.Add("frmPlaneamiento_View_NumeroParte", Nothing)
    ...
    Private Function GetSqlQuery(ByVal sField As String, ByVal sText As String, ByVal sText2 As String, ByVal sOrder As String) As String
        ...
        If String.IsNullOrEmpty(sCantidadRows) Then
            ' sCantidadRows = ConfigurationManager.AppSettiºngs("Default_CantRegistros")
            sCantidadRows = 10000000000000

        End If

        Dim ssql As String = "SELECT top " & sCantidadRows & " COSTOS.id_costo, CASE WHEN SUBSTRING(costos.OBSERVACIONES,1,7)='Cosecha' then 'Cosecha' else TIPO_COSTO.descripcion end as tipocosto, COSTOS.numero_parte, COSTOS.numero_parte as presup, COSTOS.fecha, CULTIVOS.descripcion as cultivo, ZONA_TRABAJO.descripcion as zt, ETAPAS.descripcion as etapa, ACTIVIDADES.descripcion as actividad, PRODUCTOS.descripcion as producto, PERSONAL.nombre as trabajador, MAQUINAS.descripcion as maquina, case when tipo_costo='c' and substring(costos.observaciones,1,7) <>'cosecha' then proveedores.descripcion else '' end as nombreprov, COSTOS.cantidad, COSTOS.costo_unitario_standar, COSTOS.monto_standar as tot, COSTOS.campana, case when cOSTOS.TC=0 then '" & Resource1.str10004 & "' else '" & Resource1.str10005 & "' end AS moneda, COSTOS.Tipo_Cambio, COSTOS.Precio_contable,costos.id_enlace AS Avance , COSTOS.id_zonatrabajo, COSTOS.observaciones, COSTOS.id_usuario, COSTOS.fechaauditoria, COSTOS.tipo_costo FROM ((CULTIVOS INNER JOIN ZONA_TRABAJO ON CULTIVOS.id_cultivo = ZONA_TRABAJO.id_cultivo) INNER JOIN (PRODUCTOS INNER JOIN (PERSONAL INNER JOIN ((ETAPAS INNER JOIN ACTIVIDADES ON ETAPAS.id_etapa = ACTIVIDADES.id_etapa) INNER JOIN (TIPO_COSTO INNER JOIN (MAQUINAS INNER JOIN COSTOS ON MAQUINAS.id_maquinaria = COSTOS.id_maquinaria) ON TIPO_COSTO.id_tipocosto = COSTOS.tipo_costo) ON ACTIVIDADES.id_actividad = COSTOS.id_actividad) ON PERSONAL.id_personal = COSTOS.id_personal) ON PRODUCTOS.id_producto = COSTOS.id_producto) ON ZONA_TRABAJO.id_zonatrabajo = COSTOS.id_zonatrabajo) inner join proveedores on costos.id_proveedor=proveedores.id_proveedor where costos.tipo_costo IN ('H','I','R','M','O')  and precio_contable<0 "
    ...
    Private Sub LoadPageResources()
        ...
        hdnStr89.Value = Resource1.str89
